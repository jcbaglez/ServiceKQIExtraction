...
def gettingFramesThread(thread,name2Save,showImages=False):
    ...
    print("Get Responsivity 4")
    results = getResponsivity4(mouse,np.asarray(array), np.asarray(times))
    #print(results)
    #results2 = getResponsivity3(mouse,np.asarray(array), np.asarray(times))
    #print(results2)
    #np.save(name2Save + "_results2",np.asarray(results2))
...
def imgFreeze(img,t):
    th = 0.95*img[0].size # Change threshold to take into account when there is a move
    fI = -1
    fE = -1
    tI = -1
    tE = -1
    ...
    for i in range (1,img.shape[0]):
        ...
        if(eq >= th):
            ...
            if (i == img.shape[0]): # It means that it is the last element
                fE = i 
                tE = t[i]
                data.append([fI,fE,tI,tE])
        else:
...
def serverCapture(t,q):
    base ="http://192.168.0.57:5000"
    header =  {'content-type' : 'application/json'}
    url = "/frame/client?t="+str(t)
    d = dict()
    r = requests.post(base + url,data = json.dumps(d), timeout= int(t)+30 ,headers=header)
    q.put(r.json())
...
def getResponsivity4(times,img,imgt):
    ...
    for i in range(len(indexs)):
        ...
        (data,df) = imgChange2(img[fI:lI],imgt[fI:lI])
#        if(i == len(indexs) -1):
#            (data,df) = imgChange2(img[fI:], imgt[fI:])
#        else:
#            (data,df) = imgChange2(img[fI:indexs[i+1]], imgt[fI:indexs[i+1]])
    ...
    for x in range(len(times)):
        #try:
            #latency.append(imgGr[x][0][0]*delta)
        latency.append(calculateLatency(delta,times[x],imgt[indexsL[x][0]: indexsL[x][1]],imgGr[x]))
        #except:
        #    latency.append(-1)
    return np.asarray(latency)
...
def getDeltas(imgt):
    ...
    return deltas
def calculateLatency(delta,t0,imgt,imgGr):
    try:
        print(len(imgGr))
        ...
        print("Something was wrong in latency calculation")
        latency = 0
    return latency
...
def getStalls(name,fps):

    # Load data
    (img,imgt) = loadData(name)
    info = getFPS(imgt)
    if(info['avgFPS'] > fps):
        (img,imgt) = diemadoFPS(name,fps)
...
def getSummaryLatency(data):
    ...
    maxV = np.max(data)
    minV = np.min(data)
    percent25 = np.percentile(data, 25, axis = 0)
    percent50 = np.percentile(data,50,axis = 0)
    percent75 = np.percentile(data,75,axis=0)
...
def plotDataDistribution(data):
    ...
    cuenta, cajas, ignorar = plt.hist(data, 20)
...
# #print(getCaptureFPS("1595853253"))
#name = "1595853253"
# t = np.load(name+'_timestamp.npy')
# print(getFPS(t))
# t= normalizedDeltaFrames(name)
# print(getFPS(t))
# #print(t)
# diemadoFPS(name, 30)

#print(getStalls(name,30))
